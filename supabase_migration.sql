...
-- Add Unique Constraint to prevent multiple active waiting attempts for a single order
CREATE UNIQUE INDEX IF NOT EXISTS idx_single_waiting_attempt 
ON public.payment_attempts (order_id) 
WHERE status = 'waiting';

-- ==============================================================================
-- 15. PROMOTIONAL CAMPAIGNS & CAMPAIGN PRODUCTS
-- ==============================================================================

-- 15.1 Promotional Campaigns Table
CREATE TABLE IF NOT EXISTS public.promotional_campaigns (
    ...
    coupon_code TEXT,
    coupon_type TEXT DEFAULT 'percentage' CHECK (coupon_type IN ('percentage', 'fixed', 'free_item')),
    coupon_value NUMERIC DEFAULT 0,
    coupon_min_order NUMERIC DEFAULT 0,
    coupon_auto_apply BOOLEAN DEFAULT false,
    coupon_free_item_id TEXT,
    coupon_free_item_quantity INTEGER DEFAULT 1,
    coupon_gift_url TEXT,
    created_at TIMESTAMP WITH TIME ZONE DEFAULT now(),
    updated_at TIMESTAMP WITH TIME ZONE DEFAULT now()
);
...
ALTER TABLE public.promotional_campaigns ADD COLUMN IF NOT EXISTS banner_image TEXT;
ALTER TABLE public.promotional_campaigns ADD COLUMN IF NOT EXISTS start_at TIMESTAMP WITH TIME ZONE DEFAULT now();
ALTER TABLE public.promotional_campaigns ADD COLUMN IF NOT EXISTS end_at TIMESTAMP WITH TIME ZONE;
ALTER TABLE public.promotional_campaigns ADD COLUMN IF NOT EXISTS status TEXT DEFAULT 'active';
ALTER TABLE public.promotional_campaigns ADD COLUMN IF NOT EXISTS is_flash_deal BOOLEAN DEFAULT false;
ALTER TABLE public.promotional_campaigns ADD COLUMN IF NOT EXISTS priority INTEGER DEFAULT 1;
ALTER TABLE public.promotional_campaigns ADD COLUMN IF NOT EXISTS coupon_code TEXT;
ALTER TABLE public.promotional_campaigns ADD COLUMN IF NOT EXISTS coupon_type TEXT DEFAULT 'percentage';
ALTER TABLE public.promotional_campaigns ADD COLUMN IF NOT EXISTS coupon_value NUMERIC DEFAULT 0;
ALTER TABLE public.promotional_campaigns ADD COLUMN IF NOT EXISTS coupon_min_order NUMERIC DEFAULT 0;
ALTER TABLE public.promotional_campaigns ADD COLUMN IF NOT EXISTS coupon_auto_apply BOOLEAN DEFAULT false;
ALTER TABLE public.promotional_campaigns ADD COLUMN IF NOT EXISTS coupon_free_item_id TEXT;
ALTER TABLE public.promotional_campaigns ADD COLUMN IF NOT EXISTS coupon_free_item_quantity INTEGER DEFAULT 1;
ALTER TABLE public.promotional_campaigns ADD COLUMN IF NOT EXISTS coupon_gift_url TEXT;
ALTER TABLE public.promotional_campaigns ADD COLUMN IF NOT EXISTS created_at TIMESTAMP WITH TIME ZONE DEFAULT now();
ALTER TABLE public.promotional_campaigns ADD COLUMN IF NOT EXISTS updated_at TIMESTAMP WITH TIME ZONE DEFAULT now();

-- 15.2 Campaign Products Table with Non-Cascading Foreign Key Integrity
CREATE TABLE IF NOT EXISTS public.campaign_products (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    campaign_id UUID NOT NULL,
    product_id UUID NOT NULL,
    campaign_price NUMERIC, -- Custom campaign offer price
    custom_image TEXT,     -- Custom uploaded product image for campaign
    custom_name TEXT,      -- Custom display name for campaign
    created_at TIMESTAMP WITH TIME ZONE DEFAULT now(),
    updated_at TIMESTAMP WITH TIME ZONE DEFAULT now(),

    -- Deleting a campaign removes its junction links in campaign_products
    CONSTRAINT fk_campaign_products_campaign 
        FOREIGN KEY (campaign_id) 
        REFERENCES public.promotional_campaigns(id) 
        ON DELETE CASCADE,

    -- ON DELETE RESTRICT guarantees that deleting a campaign product never deletes the main product!
    -- Also ensures products in products table cannot be accidentally dropped while assigned to a campaign.
    CONSTRAINT fk_campaign_products_product 
        FOREIGN KEY (product_id) 
        REFERENCES public.products(id) 
        ON DELETE RESTRICT,

    CONSTRAINT uq_campaign_product_pair UNIQUE (campaign_id, product_id)
);

-- Ensure columns exist if table was partially created
ALTER TABLE public.campaign_products ADD COLUMN IF NOT EXISTS campaign_id UUID;
ALTER TABLE public.campaign_products ADD COLUMN IF NOT EXISTS product_id UUID;
ALTER TABLE public.campaign_products ADD COLUMN IF NOT EXISTS campaign_price NUMERIC;
ALTER TABLE public.campaign_products ADD COLUMN IF NOT EXISTS custom_image TEXT;
ALTER TABLE public.campaign_products ADD COLUMN IF NOT EXISTS custom_name TEXT;
ALTER TABLE public.campaign_products ADD COLUMN IF NOT EXISTS created_at TIMESTAMP WITH TIME ZONE DEFAULT now();
ALTER TABLE public.campaign_products ADD COLUMN IF NOT EXISTS updated_at TIMESTAMP WITH TIME ZONE DEFAULT now();

-- 15.3 Indexes for High-Performance Queries
CREATE INDEX IF NOT EXISTS idx_promotional_campaigns_status_dates ON public.promotional_campaigns(status, start_at, end_at);
...
-- 15.4 Enable Row Level Security (RLS)
ALTER TABLE public.promotional_campaigns ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.campaign_products ENABLE ROW LEVEL SECURITY;

-- 15.5 Policies for Promotional Campaigns
DROP POLICY IF EXISTS "Public Read Promotional Campaigns" ON public.promotional_campaigns;
CREATE POLICY "Public Read Promotional Campaigns" 
ON public.promotional_campaigns 
FOR SELECT 
USING (true);

DROP POLICY IF EXISTS "Admin Manage Promotional Campaigns" ON public.promotional_campaigns;
CREATE POLICY "Admin Manage Promotional Campaigns" 
ON public.promotional_campaigns 
FOR ALL 
USING (true) 
WITH CHECK (true);

-- 15.6 Policies for Campaign Products (Enforcing Integrity Without Cascading Deletes)
DROP POLICY IF EXISTS "Public Read Campaign Products" ON public.campaign_products;
CREATE POLICY "Public Read Campaign Products" 
ON public.campaign_products 
FOR SELECT 
USING (true);

DROP POLICY IF EXISTS "Admin Manage Campaign Products" ON public.campaign_products;
CREATE POLICY "Admin Manage Campaign Products" 
ON public.campaign_products 
FOR ALL 
USING (
    -- RLS check: product must exist in the main products table
    EXISTS (
        SELECT 1 FROM public.products p 
        WHERE p.id = campaign_products.product_id
    )
) 
WITH CHECK (
    -- RLS write check: product must exist in the main products table
    EXISTS (
        SELECT 1 FROM public.products p 
        WHERE p.id = campaign_products.product_id
    )
...
-- 15.7 Auto-update Timestamps Triggers
DROP TRIGGER IF EXISTS trg_promotional_campaigns_updated ON public.promotional_campaigns;
CREATE TRIGGER trg_promotional_campaigns_updated
    BEFORE UPDATE ON public.promotional_campaigns
    FOR EACH ROW
    EXECUTE PROCEDURE update_updated_at_column();

DROP TRIGGER IF EXISTS trg_campaign_products_updated ON public.campaign_products;
CREATE TRIGGER trg_campaign_products_updated
