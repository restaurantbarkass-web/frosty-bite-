...
export const HomeHeroBanner: React.FC<HomeHeroBannerProps> = ({ onOrderNow }) => {
  ...
  // Fetch banners & campaigns from backend and listen for realtime updates
  const fetchBannersAndCampaigns = useCallback(async () => {
    try {
      const [bannerData, campaignData] = await Promise.all([
        BannerService.getBanners((updated) => {
      ...
      if (Array.isArray(bannerData)) {
        const active = bannerData.filter((b) => b.is_active !== false);
        setBackendBanners(active);
      }
      ...
      console.warn('[HomeHeroBanner] Error fetching banners/campaigns:', err);
    }
  }, []);

  useEffect(() => {
    fetchBannersAndCampaigns();
    ...
    const unsubCampaigns = CampaignService.subscribeToCampaigns(() => {
      fetchBannersAndCampaigns();
    });
  ...
  // Combine campaigns, backend banners or fallback to handcrafted default slides
  const slides: UnifiedSlide[] = useMemo(() => {
    const list: UnifiedSlide[] = [];

    // 1. Promotional Campaigns have top billing
    if (campaigns.length > 0) {
      campaigns.forEach((camp) => {
        list.push({
          ...
          isBackend: true,
          title: camp.title,
          subtitle: camp.description || (camp.coupon ? `Exclusive Offer: Apply code ${camp.coupon.code} for discounts on selected treats!` : 'Handcrafted specialty campaign items.'),
          imageUrl: camp.banner_image,
          autoApplyCoupon: camp.coupon?.code,
          redirectUrl: `/campaign/${camp.id}`,
          isFlashDeal: camp.is_flash_deal,
          ctaText: camp.coupon ? 'VIEW OFFERS' : 'EXPLORE DEALS',
          badgeText: 'CAMPAIGN OFFER',
          badgeEmoji: '🎁',
          bgGradient: 'from-[#181411] via-[#211A16] to-[#2D211B]',
          glowColor: 'bg-[#E76A54]/20',
        });
      });
    }

    // 2. Standard backend banners
    if (backendBanners.length > 0) {
      backendBanners.forEach((banner, idx) => {
        list.push({
