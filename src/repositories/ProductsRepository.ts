...
class ProductsRepositoryImpl extends BaseRepository {
  /**
   * Fetch all products using CacheOrchestrator SWR policy
   */
  async getProducts(onUpdate?: (items: FoodItem[]) => void): Promise<FoodItem[]> {
    const data = await this.fetchWithCache<FoodItem[]>(
      CacheKeys.PRODUCTS,
      async () => {
  ...
  /**
   * Get single product with cache fallback
   */
  async getProductById(id: string): Promise<FoodItem | null> {
    const products = await this.getProducts();
    const found = products.find((p) => p.id === id);
    if (found) return found;

    return this.deduplicate(`product_${id}`, async () => {
      try {
        const { data, error } = await supabase
          .from('products')
          .select('*')
  ...
  private mapProductRecord(item: any): FoodItem {
