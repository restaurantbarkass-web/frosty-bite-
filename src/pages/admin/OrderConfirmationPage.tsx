...
export const OrderConfirmationPage: React.FC<OrderConfirmationPageProps> = ({
  ...
  const trackingLink = `${window.location.origin}/order-tracking/${displayOrder.id}`;
  const defaultMessage = buildOrderConfirmationWhatsAppMessage(displayOrder, trackingLink);
  const [customMessage, setCustomMessage] = useState(defaultMessage);

  useEffect(() => {
    const freshLink = `${window.location.origin}/order-tracking/${displayOrder.id}`;
    setCustomMessage(buildOrderConfirmationWhatsAppMessage(displayOrder, freshLink));
  }, [displayOrder]);
  ...
  const handleOpenWhatsApp = () => {
    ...
    try {
      ...
      if (win) {
        ...
        toast.success('✓ WhatsApp opened with the confirmation message', {
          style: {
            background: '#121212',
            color: '#22c55e',
  ...
  return (
    <div className={`min-h-screen bg-[#070709] text-white selection:bg-orange-500/30 font-sans p-4 sm:p-6 lg:p-8 relative overflow-x-hidden ${isStandalonePage ? 'pt-6' : ''}`}>
      {/* Background Ambient Glows */}
      <div className="fixed top-0 left-1/4 w-96 h-96 bg-orange-500/10 rounded-full blur-[120px] pointer-events-none" />
      ...
      <div className="max-w-6xl mx-auto space-y-6 relative z-10">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
          ...
          <div className="lg:col-span-7 space-y-6">
            ...
            {/* Main Action Box */}
            <div className="bg-[#0f0f13] border border-orange-500/40 rounded-3xl p-6 sm:p-7 shadow-2xl space-y-6 relative overflow-hidden">
              ...
              <div className="flex items-center justify-between border-b border-white/10 pb-4">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-zinc-400">Send confirmation & live tracking link to {customerName}</p>
                  </div>
                </div>
              ...
              {hasValidPhone ? (
                <div className="space-y-4">
                  ...
                  >
                    ...
                    <span>📱 {hasOpenedWhatsApp ? 'WhatsApp Opened — Send Again' : 'Send Order Confirmation'}</span>
                ...
                {isEditingMessage ? (
                  <div className="space-y-3">
                    ...
                    <div className="flex flex-wrap gap-1.5 pt-1">
                      ...
                      <button
                        type="button"
                        onClick={() => appendSnippet('🎂 *Note:* Custom requests and greetings have been carefully noted by our baking chefs!')}
                      ...
                      <button
                        type="button"
                        onClick={() => appendSnippet('🚗 *Rider Detail:* Once dispatched, your rider contact details will be shared on the tracking link.')}
                        className="px-2.5 py-1 rounded-lg bg-emerald-950/60 hover:bg-emerald-900/60 border border-emerald-500/30 text-emerald-300 text-[10px] font-bold cursor-pointer"
                      >
                        + Dispatch Info
                      </button>
          ...
          {/* Right Column (5 cols): Order Details & Confirmation Summary */}
          <div className="lg:col-span-5 space-y-6">
            ...
            {/* Audit Log / Verification details */}
            <div className="bg-[#0f0f13] border border-orange-500/20 rounded-3xl p-6 shadow-2xl space-y-4">
              <div className="flex items-center gap-2 text-orange-400">
                <Truck size={18} />
                <h3 className="text-sm font-extrabold uppercase tracking-wider">Order Status Audit</h3>
              </div>
              ...
              <div className="space-y-2 text-xs">
                ...
                <div className="flex items-center justify-between text-zinc-400 py-1 border-b border-white/5">
                  <span>Secure Live Tracker:</span>
                  <span className="text-emerald-400 font-bold flex items-center gap-1">
                    <ShieldCheck size={13} />
                    Dynamic Link Embedded
                  </span>
                </div>
