...
export const Checkout: React.FC = () => {
  ...
  const eligibleCampaignSubtotal = appliedCoupon?.eligible_product_ids && appliedCoupon.eligible_product_ids.length > 0
    ? cart
        .filter(item => appliedCoupon.eligible_product_ids!.includes(item.id))
        .reduce((sum, item) => sum + item.price * item.quantity, 0)
    : subtotal;

  const discountAmount = appliedCoupon 
    ? (appliedCoupon.eligible_product_ids && appliedCoupon.eligible_product_ids.length > 0
        ? (eligibleCampaignSubtotal <= 0 
            ? 0 
            : (appliedCoupon.type === 'percentage' 
                ? Math.round((eligibleCampaignSubtotal * appliedCoupon.value) / 100) 
                : Math.min(eligibleCampaignSubtotal, appliedCoupon.value)))
        : (appliedCoupon.type === 'percentage' 
  ...
  const handleApplyCoupon = async (codeOverride?: string) => {
    ...
    try {
      ...
      // Strict server-side / campaign validation
      const validation = await CampaignService.validateCoupon(
        code,
        cart.map(item => ({ id: item.id, price: item.price, quantity: item.quantity }))
      );

      if (!validation.valid) {
        toast.error(validation.message || 'Invalid or expired coupon code');
        setIsApplyingCoupon(false);
        return;
      }

      const couponId = validation.coupon?.id || `coupon-${code}`;
      const isCampaignLocked = !!(validation.eligibleProductIds && validation.eligibleProductIds.length > 0);

      setAppliedCoupon({ 
        id: couponId,
        code: validation.coupon?.code?.toUpperCase() || code, 
        value: Number(validation.coupon?.value) || 0,
        type: validation.coupon?.type || 'percentage',
        campaign_id: validation.campaignId || null,
        eligible_product_ids: validation.eligibleProductIds || null,
        campaign_title: validation.campaignTitle || null,
        free_item_id: validation.coupon?.free_item_id,
        free_item_quantity: validation.coupon?.free_item_quantity,
        gift_url: validation.coupon?.gift_url
      });

      let discountDisplay = '';
      if (validation.coupon?.type === 'percentage') discountDisplay = `${validation.coupon.value}% OFF`;
      else if (validation.coupon?.type === 'fixed') discountDisplay = `₹${validation.coupon.value} OFF`;
      else if (validation.coupon?.type === 'free_item') discountDisplay = 'FREE GIFT';

      if (isCampaignLocked) {
        toast.success(`🎉 Campaign Offer "${code}" Applied! (${discountDisplay} on eligible items)`);
      } else {
        toast.success(`🎉 Coupon "${code}" Applied! (${discountDisplay})`);
      }
      ...
      console.warn('[Checkout] Coupon validation error:', err);
                  ...
                  <div key={item.id} className="flex gap-3.5 items-center group">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-1.5">
                        ...
                        {appliedCoupon?.eligible_product_ids?.includes(item.id) && (
                ...
                {appliedCoupon && (
                  <div className="space-y-0.5">
                    <div className="flex justify-between items-center text-xs font-semibold text-emerald-600">
                      <span>Discount ({appliedCoupon.code}{appliedCoupon.campaign_title ? ` • ${appliedCoupon.campaign_title}` : ''})</span>
                      <span>-₹{discountAmount}</span>
                    </div>
                    {appliedCoupon.eligible_product_ids && appliedCoupon.eligible_product_ids.length > 0 && (
                      <p className="text-[10px] text-stone-400 text-left">
                        * Applied strictly to eligible campaign items in your cart
                      </p>
                    )}
                  </div>
                )}
