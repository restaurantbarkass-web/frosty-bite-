...
export async function getRecommendation(req: Request, res: Response) {
  ...
  try {
    ...
    const fallback = await recommendationService.getSmartRecommendation(query || 'bakery', items || []);
    res.json(fallback);
  }
}
...
export async function getSuggestions(req: Request, res: Response) {
  ...
  try {
    ...
    const fallback = await recommendationService.getSearchSuggestions(searchTerm || '', items || []);
