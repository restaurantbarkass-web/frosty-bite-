...
var emgPath = path2.resolve(process.cwd(), ".env.example");
if (fs2.existsSync(envPath)) {
  dotenv.config({ path: envPath });
}
if (process.env.NODE_ENV !== "production" && fs2.existsSync(emgPath)) {
  try {
    const exampleConfig = dotenv.parse(fs2.readFileSync(emgPath));
...
app.use(express7.urlencoded({ limit: "10mb", extended: true }));
...
app.get("/migration-script", (req, res) => {
  try {
    const migrationPath = path2.resolve(process.cwd(), "supabase_migration.sql");
    if (fs2.existsSync(migrationPath)) {
      const sqlText = fs2.readFileSync(migrationPath, "utf-8");
...
app.use(["/v2", "/api/v2", "/api/geofencing", "/geofencing"], v2geofencing_routes_default);
...
// api/index.ts
var envPath2 = path3.resolve(process.cwd(), ".env");
